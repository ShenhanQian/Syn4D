...
#
# Batch convert SMPL-X Alembic .abc animations to per-animation .npz files.
...
SUCCESS_LOG_MARKERS = ("Saved NPZ sequence:", "Finished. Time:")
WORKER_SCRIPT = Path(__file__).with_name("smplx_anim_to_objs.py")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Batch convert SMPL-X Alembic .abc files to per-animation .npz files.",
    )
    parser.add_argument("input_dir", type=Path, help="Directory containing Alembic .abc files.")
    parser.add_argument("output_dir", type=Path, help="Directory where vertex-cache .npz files and logs will be written.")
    ...
    parser.add_argument(
        "--skip-existing",
        action="store_true",
        help="Skip output .npz files that already exist and have nonzero size.",
    )
    parser.add_argument(
        "--stop-file",
        type=Path,
...
def output_path_for(input_path: Path, output_dir: Path) -> Path:
    if input_path.name.startswith("moyo"):
        return output_dir / "moyo" / f"{input_path.stem}.npz"
    body_name = input_path.name.rsplit("_", maxsplit=1)[0]
    return output_dir / body_name / f"{input_path.stem}.npz"
...
def build_tasks(input_dir: Path, output_dir: Path, skip_existing: bool) -> list[ConversionTask]:
    ...
    for input_path in sorted(input_dir.rglob("*.abc")):
